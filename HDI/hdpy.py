...
import numpy as np
import math
from sklearn.linear_model import Lasso,LassoCV,RidgeCV,lasso_path,LassoLars,LogisticRegressionCV
...
class multi_split:
        ...
        while tryagain:
            ...
            if p_sel > 0 and (p_sel+1) < nright:
                ...
                if len(sel_pval) != p_sel:
                    sys.exit("The statsmodels.OLS didn't return the correct number of p-values for the provided submodel.")
    ...
    def fit(self,X,y):
        ...
        for b in range(self.B):
            pvals[b,:], s0[b], coefs[b,:], lci[b,:], uci[b,:], ses[b,:], df_res[b]= h.apply_async(self.singlesplit, args=(X,y,nleft)).get()
        h.close()
        h.join()
...
class lasso_proj:
    def __init__(self, ci = True, ci_level = 0.95, standardize=True,robust=True,family = "gaussian",verbose = True):
...
def hd(method="multi-split", X=None, y=None, family="gaussian",
       ci=True, ci_level=0.95,B=100,fraction=0.5,repeat_max = 20,manual_lam = True,
                aggr_ci = True, return_nonaggr = False,alpha=None, intercept=False,
       model_selector=LassoCV(cv=10,n_jobs=-1,fit_intercept=False,tol= 0.0001),
       robust=True,standardize=True,
       ridge_unprojected=False):
        ...
        if method == "multi-split" or method == "multi":
            est = multi_split(ci=ci, ci_level = ci_level,B=B,fraction=fraction,manual_lam = manual_lam,aggr_ci = aggr_ci, return_nonaggr = return_nonaggr,model_selector=model_selector,repeat_max = repeat_max)
             est.fit(X,y)

        if method == "lasso-proj" or method == "lassoproj":
            est = lasso_proj(ci=ci, ci_level = ci_level, standardize=standardize, robust=robust,family = family,verbose = True)
            est.fit(X,y)

        if method == "ridge-proj" or method == "ridgeproj":
            est = ridge_proj(ci=ci, ci_level = ci_level,standardize=True,ridge_unprojected=False,family="gaussian")
            est.fit(X,y)

        if method == "debiasedLasso" or method == "debiased":
            est = debiasedLasso(ci=ci, ci_level = ci_level)
            est.fit(X,y)
