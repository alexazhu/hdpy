...
class multi_split:
    def __init__(self,B=100,fraction=0.5,model_selector="LassoCV",ci=True,ci_level=0.95,exact=True,repeat_max = 20):
        ...
        self.selector = LassoCV(cv=10,n_jobs=-1,fit_intercept=False)
    ...
    def control_FDR(self,q):
        ## FDR control at q level
        order = np.argsort(self.pvals_corr)
        pval_order = np.sort(self.pvals_corr) #ascending
        index = np.arange(1,self.d+1,1,dtype=int)
        h = np.max(np.where(self.pvals_corr <= index * q/np.sum(1/index))[0])
        active_FDR = np.where(self.pvals_corr <= pval_order[h])[0]
        return active_FDR

    def control_FWER(self,level):
        active_FWER =np.where(self.pvals_corr <= level)[0]
        return active_FWER
