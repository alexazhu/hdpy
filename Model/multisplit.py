...
class Multisplit:    
    def __init__(self, ci=False, ci_level=0.95,
        ...
        ''' This class is an implementation of Multiple Splitting method https://doi.org/10.1198/jasa.2009.tm08647
        
        Our implementation has refered to the R package hdi by Lukas Meier, Date:  2 Apr 2013, 11:52
        ==========================================================================================
        Parameters:
        ...
        while tryagain:
            ...
            if self.manual_lam:
                ...
                lambda_path = np.round(np.exp(np.linspace(math.log10(max_lambda), math.log10(max_lambda * eps), K)),
                                       decimals=100)
                model_selector.set_params(alphas=lambda_path,normalize =True,tol=1e-3)

            #print(lambda_path)
            model_selector.fit(X=xleft, y=yleft)

            sel_nonzero = (model_selector.coef_ != 0)  # location of selected variables
    ...
    def fit(self, X, y, manual_lam=True,model_selector=LassoCV(n_jobs=-1, fit_intercept=False, tol=1e-07,cv=10)):
        '''       
        ==========================================================================================
        ...
        for b in range(self.B):
            pvals[b, :], s0[b], coefs[b, :], lci[b, :], uci[b, :], ses[b, :], df_res[b] = self.singlesplit(X, y, nleft, model_selector)
