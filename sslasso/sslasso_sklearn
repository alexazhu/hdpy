...
class debiasedLasso:
    ...
    def noiseEstimator(self, b, A, n):  # implementation translated from R code in the p-value Lasso paper
    ...
    def inverseLinftyOneRow(self,sigma, i, mu, maxiter=50, threshold=1e-2):
        ...
        while (iter <= maxiter) and (diff_norm2 >= (threshold * last_norm2)):
            for j in range(p):
                ...
                if j == i:
                    v = v + 1
                beta[j] = (np.sign(v) * max(0, abs(v) - mu)) / sigma[j, j]
                if oldval != beta[j]:
    ...
    def fit(self, X, y):
        ...
        M = self.inverseLinfty(sigma=sigma_hat, n=n, resol=1.5, mu=None, maxiter=50, threshold=1e-2)
        ...
        thetamodel = LassoCV(fit_intercept=False)
        thetamodel.fit(X,y)
        ...
        beta = htheta + np.dot(np.dot(M, Xb.T), r) / n
        # beta = beta + 1.0/n*np.dot(X.T, r)
        beta = beta.reshape((p,))

        sdhat, nz = self.noiseEstimator(beta, A, n)
        unbiased_lasso = beta * col_norm
